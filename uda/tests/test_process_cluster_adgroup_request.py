...
def run_process_request(repo_root: Path, request_file: Path, config_file: Path, output_dir: Path):
    cmd = [
        sys.executable,
        str(repo_root / "uda" / "scripts" / "process_cluster_adgroup_request.py"),
        "--request-file",
        str(request_file),
        "--config-file",
        str(config_file),
...
def seed_repo_layout(repo_root: Path):
    ...
    source_script = Path(__file__).parents[1] / "scripts" / "process_cluster_adgroup_request.py"
    source_config = Path(__file__).parents[1] / "config" / "environments.yaml"

    (repo_root / "uda" / "scripts" / "process_cluster_adgroup_request.py").write_text(
