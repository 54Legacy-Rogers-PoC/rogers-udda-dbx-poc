#!/usr/bin/env python3
"""Validate the live Databricks cluster permission state against the requested service-account manifest."""
...
def _has_expected_permission(payload: dict[str, Any], service_account_name: str, permission_level: str) -> bool:
    desired_principal = _principal_name(service_account_name)
    desired_level = _normalize(permission_level).upper()

    for entry in _access_control_entries(payload):
        current_principal = _principal_name(
            entry.get("service_principal_name") or entry.get("principal_name") or entry.get("user_name")
        )
        if current_principal != desired_principal:
            continue
...
def validate_manifest(manifest_payload: dict[str, Any], databricks_host: str, databricks_token: str) -> int:
    ...
    service_account_name = _normalize(manifest_payload.get("service_account_name"))
    permission_level = _normalize(manifest_payload.get("cluster_permission_level") or "CAN_ATTACH_TO").upper()
    activity_type = _normalize(manifest_payload.get("activity_type") or "ADD_TO_CLUSTER").upper()

    if not cluster_id or not service_account_name:
        print("Manifest is missing cluster_id/cluster_name or service_account_name.", file=sys.stderr)
        return 2
    ...
    has_permission = _has_expected_permission(payload, service_account_name, permission_level)

    if activity_type == "REMOVE_FROM_CLUSTER":
        if not has_permission:
            print("Live Databricks cluster permissions are absent as expected for this REMOVE_FROM_CLUSTER request.")
            return 0
        print(
            f"Validation failed because {service_account_name} still has {permission_level} on cluster {cluster_id}.",
            file=sys.stderr,
        )
        return 1
    ...
    if has_permission:
        print(
            f"Live Databricks cluster permissions include {service_account_name} with {permission_level} on cluster {cluster_id}."
        )
        return 0

    print(
        f"Validation failed because {service_account_name} does not have {permission_level} on cluster {cluster_id}.",
        file=sys.stderr,
    )
    return 1
